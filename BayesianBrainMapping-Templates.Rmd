...
<pre> ```text BayesianBrainMapping-Templates/ ├── data/ │ ├── template_rds/ │ ├── parcellations_plots/ │ └── ... ├── src/ │ ├── 0_setup.R │ ├── 1_fd_time_filtering.R │ └── ... ├── BayesianBrainMapping-Templates.Rmd └── README.md ``` </pre>
...
[@Nohelia, insert starting number], we apply the following filtering steps:
...
[@Nohelia -- and for both visits also?] This list is referred to as the `combined` 
...
exclude related individuals. [@Nohelia very important that we don't share the list
of final subjects included, because that could reveal which subjects are related.  Can you
confirm we do not share that information?]
