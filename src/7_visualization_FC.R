...
for (file in prior_files) {
    ...
    # Save as PDF (2 pages)
    pdf(file.path(dir_data, "outputs", paste0(base_name, "_FC_Cholesky.pdf")), height=5, width=5.5)
    plot_FC(prior_FC_mean_Cholesky, zlim=zlim_FC, title = paste0(plot_title, "\nCholesky FC Prior Mean")) # nolint
    plot_FC(sqrt(prior_FC_var_Cholesky), zlim=c(0.1, 0.3), cols = viridis(12), title = paste0(plot_title, "\nCholesky FC Prior SD")) # nolint
    dev.off()

    # Save as png (2 images)
    png(file.path(dir_data, "outputs", paste0(base_name, "_FC_Cholesky_mean.png")), height = 550, width = 600)
    plot_FC(prior_FC_mean_Cholesky, zlim = zlim_FC, title = paste0(plot_title, "\nCholesky FC Prior Mean"))
    dev.off()

    png(file.path(dir_data, "outputs", paste0(base_name, "_FC_Cholesky_sd.png")), height = 550, width = 600)
    ...
    # Save as PDF (2 pages)
    pdf(file.path(dir_data, "outputs", paste0(base_name, "_FC_InverseWishart.pdf")), height=5, width=5.5)
    ...
    # Save as png (2 images)
    png(file.path(dir_data, "outputs", paste0(base_name, "_FC_InverseWishart_mean.png")), height = 550, width = 600)
    plot_FC(prior_FC_mean_InverseWishart, zlim = zlim_FC, title = paste0(plot_title, "\nInverse-Wishart FC Prior Mean"))
    dev.off()

    png(file.path(dir_data, "outputs", paste0(base_name, "_FC_InverseWishart_sd.png")), height = 550, width = 600)
    plot_FC(sqrt(prior_FC_var_InverseWishart), zlim = c(0.1, 0.3), cols = viridis(12), title = paste0(plot_title, "\nInverse-Wishart FC Prior SD"))
    dev.off()
}
